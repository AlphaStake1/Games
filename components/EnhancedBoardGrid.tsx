...
const EnhancedBoardGrid: React.FC<EnhancedBoardGridProps> = ({
  ...
  const [homeNumbers] = useState<string[]>(['R', 'A', 'N', 'D', 'O', 'M', 'I', 'Z', 'E', 'D']);
  const [awayNumbers] = useState<string[]>(['R', 'A', 'N', 'D', 'O', 'M', 'I', 'Z', 'E', 'D']);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setSquares(currentSquares => {
        const availableSquares = currentSquares.filter(s => !s.isOwned && !s.isAnimating);
            ...
            setSquares(prevSquares => {
              ...
              if (updatedSquares[squareIndex]?.isAnimating) {
                updatedSquares[squareIndex] = {
                  ...
                  isOwned: true,
                  owner: 'other-user',
                  ownerNFT: NFT_SYMBOLS[Math.floor(Math.random() * NFT_SYMBOLS.length)],
                  isAnimating: false
                };
              }
  ...
  useEffect(() => {
    const initialSquares: SquareState[] = Array.from({ length: 100 }, (_, index) => {
      const isOwned = Math.random() < board.totalSquaresSold / 100;
      return {
        index,
        isOwned,
        owner: isOwned ? (Math.random() < 0.3 ? userWalletAddress || undefined : 'other-user') : undefined,
        ownerNFT: isOwned ? NFT_SYMBOLS[Math.floor(Math.random() * NFT_SYMBOLS.length)] : undefined,
        isSelected: false,
        isWinning: false,
  ...
  const handleSquareClick = useCallback((squareIndex: number) => {
    if (!userWalletAddress || squares[squareIndex]?.isOwned || squares[squareIndex]?.isAnimating) return;
  ...
  }, [squares, selectedSquares, maxSquares, userWalletAddress, board, onSquareSelectionChange, isVIP]);
  ...
  const getSquareBackgroundColor = (square: SquareState): string => {
    ...
    if (square.isAnimating) return "bg-yellow-200 animate-pulse cursor-not-allowed";
  ...
  const getSquareContent = (square: SquareState): JSX.Element => {
    ...
    if (square.isOwned) return <span className="text-lg">{square.ownerNFT}</span>;
    if (selectedSquares.includes(square.index)) return <CheckCircle className="w-6 h-6 text-white" />;
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                {board.game.homeTeam.city} vs {board.game.awayTeam.city} • Week {board.game.week}
              </p>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="text-center">
            ...
            <p className="text-lg font-bold text-blue-600">{availableSquares}</p>
          </div>
          <div className="text-center">
            <div className="flex items-center justify-center gap-1 mb-1">
            ...
            <p className="text-lg font-bold text-gray-600">{100 - availableSquares}</p>
          </div>
          <div className="text-center">
            <div className="flex items-center justify-center gap-1 mb-1">
            ...
            <p className="text-lg font-bold text-purple-600">{selectedSquares.length}</p>
          </div>
        </div>
        ...
        <div className="w-full">
            ...
            <div className="flex">
                <div className="w-24 shrink-0 h-16 flex flex-col items-center justify-center bg-white text-black text-sm font-bold border-r border-b border-gray-400">
                    <div className="text-xs">{board.gameStartTime.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</div>
            ...
            <div className="flex">
                ...
                <div className="flex-1 flex flex-col">
                    ...
                    <div className="flex">
                        ...
                        {homeNumbers.map((num) => (
                            <div key={`home-${num}`} className="flex-1 aspect-square flex items-center justify-center bg-blue-200 text-black font-bold border-b border-r border-gray-400">
                                {num}
                            </div>
                        ))}
                    </div>
                            ...
                            {homeNumbers.map((homeNum, colIndex) => {
                                ...
                                return (
                                    <button
                                        ...
                                        disabled={!square || square.isOwned || square.isAnimating || !board.isActive}
