...
const SquaresGrid: React.FC<SquaresGridProps> = ({
  ...
  const handleWebSocketMessage = useCallback((message: any) => {
    switch (message.type) {
      ...
      case 'score_update':
        ...
        toast.info(
          `Score update: ${message.data.homeScore}-${message.data.awayScore} Q${message.data.quarter}`,
        );
        break;
  ...
  const handleSquareClick = (squareIndex: number) => {
    ...
    if (
      boardState.squares[squareIndex] !== '11111111111111111111111111111111'
    ) {
  ...
  const getSquareColor = (squareIndex: number): string => {
    ...
    const isOwned =
      boardState.squares[squareIndex] !== '11111111111111111111111111111111';
    const isUserSquare =
      boardState.squares[squareIndex] === publicKey?.toString();
    const isWinningSquare =
      boardState.winner &&
      boardState.squares[squareIndex] === boardState.winner;
  ...
  const renderGameInfo = () => {
    ...
    const soldSquares = boardState.squares.filter(
      ...
      <div className="overflow-x-auto">
        ...
        <div className="grid grid-cols-11 gap-1 mb-2">
          ...
          {boardState.homeHeaders.map((header, index) => (
            <div
              key={index}
              className="w-12 h-12 flex items-center justify-center bg-blue-100 border border-blue-300 rounded font-bold"
            >
              {header}
            </div>
          ))}
        ...
        <CardContent>
          ...
          {boardState?.gameEnded && boardState.winner && (
            <div className="mt-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              ...
              <p>
                Final Score: {boardState.homeScore}-{boardState.awayScore}
              </p>
              <p>
                Payout: {(boardState.payoutAmount / 1_000_000_000).toFixed(2)}{' '}
                SOL
              </p>
            </div>
          )}
