...
exports.CalculatorAgent = exports.GAME_CONFIG = void 0;
...
class CalculatorAgent extends events_1.EventEmitter {
  constructor() {
    super();
    this.defaultPayoutDistribution = [
      { quarter: 'Q1', percentage: 0.15 },
      { quarter: 'Q2', percentage: 0.25 },
      { quarter: 'Q3', percentage: 0.15 },
      { quarter: 'Q4', percentage: 0.35 },
  ...
  calculateWinningSquare(homeScore, awayScore, homeHeaders, awayHeaders) {
    try {
      ...
      const winningSquareIndex =
        awayIndex * exports.GAME_CONFIG.BOARD_SIZE + homeIndex;
      return {
        success: true,
        result: {
          winningSquareIndex,
  ...
  calculatePotDistribution(
    squares,
    rakePercentage = exports.GAME_CONFIG.DEFAULT_RAKE_PERCENTAGE,
  ) {
    try {
  ...
  calculateQuarterlyPayouts(
    playerPot,
    payoutDistribution = this.defaultPayoutDistribution,
  ) {
    try {
   ...
   */
  calculateWinnerPayout(winningSquareIndex, squares, quarterlyAmount) {
    try {
   ...
   */
  calculateLeaderboardPoints(wins, totalWinnings, gamesParticipated) {
    try {
   ...
   */
  getSquareCoordinates(index) {
    try {
      if (index < 0 || index >= exports.GAME_CONFIG.TOTAL_SQUARES) {
        return {
          success: false,
      ...
      const row = Math.floor(index / exports.GAME_CONFIG.BOARD_SIZE);
      const col = index % exports.GAME_CONFIG.BOARD_SIZE;
      return {
        success: true,
        result: {
          index,
   ...
   */
  validateGameConfig(entryFee, rakePercentage, payoutDistribution) {
    try {
      const errors = [];
      // Validate entry fee
      if (
        entryFee < exports.GAME_CONFIG.MIN_ENTRY_FEE ||
        entryFee > exports.GAME_CONFIG.MAX_ENTRY_FEE
      ) {
        errors.push(
          `Entry fee ${entryFee} outside valid range ${exports.GAME_CONFIG.MIN_ENTRY_FEE}-${exports.GAME_CONFIG.MAX_ENTRY_FEE}`,
        );
      }
      ...
      const validQuarters = Object.values(exports.GAME_CONFIG.QUARTERS);
...
exports.CalculatorAgent = CalculatorAgent;
exports.default = CalculatorAgent;
