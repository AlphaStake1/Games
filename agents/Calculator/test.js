...
class TestRunner {
  constructor() {
    this.testResults = [];
    this.calculator = new index_1.CalculatorAgent();
  }
  ...
  testQuarterlyPayouts() {
    this.runTest('Quarterly Payouts - Default Distribution', () => {
      ...
      this.assert(
        payouts.find((p) => p.quarter === 'Q1')?.amount === 15,
        'Q1 should be 15 SOL (15%)',
      );
      this.assert(
        payouts.find((p) => p.quarter === 'Q2')?.amount === 25,
        'Q2 should be 25 SOL (25%)',
      );
      this.assert(
        payouts.find((p) => p.quarter === 'Q3')?.amount === 15,
        'Q3 should be 15 SOL (15%)',
      );
      this.assert(
        payouts.find((p) => p.quarter === 'Q4')?.amount === 35,
        'Q4 should be 35 SOL (35%)',
      );
      this.assert(
        payouts.find((p) => p.quarter === 'FINAL')?.amount === 10,
    ...
    this.runTest('Quarterly Payouts - Invalid Distribution', () => {
      ...
      const invalidDistribution = [
        { quarter: 'Q1', percentage: 0.3 },
        { quarter: 'Q2', percentage: 0.3 },
        { quarter: 'Q3', percentage: 0.3 }, // Total = 0.9, not 1.0
      ];
  ...
  testGameConfigValidation() {
    this.runTest('Game Config Validation - Valid Config', () => {
      const validDistribution = [
        { quarter: 'Q1', percentage: 0.2 },
        { quarter: 'Q2', percentage: 0.3 },
        { quarter: 'Q3', percentage: 0.2 },
        { quarter: 'Q4', percentage: 0.2 },
        { quarter: 'FINAL', percentage: 0.1 },
      ];
    ...
    this.runTest('Game Config Validation - Invalid Entry Fee', () => {
      const validDistribution = [{ quarter: 'Q1', percentage: 1.0 }];
