/**
 * Football Squares Telegram Bots - Quick Start
 * Properly handles async/await without top-level await issues
 */
...
coachBBot.on('text', async (ctx) => {
  ...
  try {
    ...
    console.log('🏈 Coach B responding...');
    await ctx.reply(reply);
  } catch (error) {
...
ocPhilBot.on('text', async (ctx) => {
  ...
  try {
    const model = gemini.getGenerativeModel({
      ...
      systemInstruction:
        "You are OC-Phil, the Offensive Coordinator who specializes in helping Community Board Leaders (CBLs) build successful football squares communities. You're strategic, analytical, and focused on growth, optimization, and community building. Provide actionable advice and specific strategies.",
    });
    ...
    console.log('📋 OC-Phil responding...');
    await ctx.reply(reply);
  } catch (error) {
...
async function startAllBots() {
  try {
    ...
    console.log('\n✅ ALL BOTS ARE NOW LIVE AND LISTENING!');
    ...
    console.log('   📋 OC-Phil (Gemini Pro) - CBL strategies');
    console.log('\n💬 Go chat with them now!');
  } catch (error) {
