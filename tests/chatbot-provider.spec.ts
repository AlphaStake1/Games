...
test.describe('ChatbotProvider Route-based Switching', () => {
  ...
  test.describe('Coach B Chatbot on Public Routes', () => {
    test('should display Coach B chatbot on homepage', async ({ page }) => {
      ...
      await expect(page.locator('text="Football Squares Assistant"')).toBeVisible();
      
      // Verify initial message is from Coach B
      await expect(page.locator('text="Hey there! I\'m Coach B"')).toBeVisible();
    });
    ...
    test('should display Coach B chatbot on /boards route', async ({ page }) => {
  ...
  test.describe('OC-Phil Widget on CBL Routes', () => {
    test('should display OC-Phil widget on /cbl/overview', async ({ page }) => {
      ...
      await expect(page.locator('text="Community Board Leader Assistant"')).toBeVisible();
      
      // Verify initial message is from OC-Phil
      await expect(page.locator('text="Welcome to the CBL Portal!"')).toBeVisible();
    });

    test('should display OC-Phil widget on /cbl/dashboard with authentication', async ({ page }) => {
  ...
  test.describe('Route Switching Behavior', () => {
    test('should switch chatbots when navigating between routes', async ({ page }) => {
  ...
  test.describe('Chatbot Functionality', () => {
    test('Coach B should respond to crypto questions', async ({ page }) => {
      ...
      const inputField = page.locator('input[placeholder="Ask Coach B anything..."]');
      ...
      // Wait for response
      await expect(page.locator('text="Great question about cryptocurrency!"')).toBeVisible({ timeout: 5000 });
    });

    test('OC-Phil should respond to CBL management questions', async ({ page }) => {
      await page.goto('/cbl/overview');
      await page.waitForLoadState('networkidle');
      ...
      const inputField = page.locator('input[placeholder="Ask OC-Phil anything..."]');
      ...
      // Wait for response
      await expect(page.locator('text="Managing boards is at the heart"')).toBeVisible({ timeout: 5000 });
    });
  });

  test.describe('Visual Consistency', () => {
    test('chatbots should maintain consistent positioning across routes', async ({ page }) => {
    ...
    test('chatbot windows should have consistent dimensions', async ({ page }) => {
