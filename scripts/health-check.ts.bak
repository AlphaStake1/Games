...
class HealthChecker {
  ...
  private async checkCeramicIntegration(): Promise<HealthCheckResult> {
    ...
    try {
      ...
      const ceramicPaths = ['ceramic/client.ts', 'lib/ceramic-integration.ts'];
  ...
  private async checkEmailSystem(): Promise<HealthCheckResult> {
    ...
    try {
      ...
      const emailAgent = readFileSync(
        join(process.cwd(), 'agents/EmailAgent/index.ts'),
        'utf8',
      );
  ...
  private printResults(results: HealthCheckResult[]): void {
    ...
    const overallStatus =
      unhealthy > 0 ? 'CRITICAL' : degraded > 0 ? 'WARNING' : 'HEALTHY';
