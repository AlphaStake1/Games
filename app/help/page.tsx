...
const RevivaBot = ({ step, issueType }: { step: number; issueType: string }) => {
  const getStepMessage = () => {
    const messages = {
      ...
      3: issueType === 'wallet' ? "Great! Which wallet are you using? (Phantom, Solflare, etc.)" :
         issueType === 'transaction' ? "Thanks! Please paste your transaction hash - this helps me diagnose the issue." :
         issueType === 'squares' ? "Nice! What's your square number or game details?" :
         issueType === 'account' ? "Got it! What's your username or account identifier?" :
    ...
    <div className="reviva-bot">
      <div className="bot-avatar">
        <div className="avatar-circle">
          ...
          />
          <span className="bot-icon" style={{ display: 'none' }}>🌱</span>
        </div>
...
const DynamicField = ({ 
  ...
  const getFieldConfig = (fieldName: string) => {
    const configs: Record<string, { label: string; type: string; placeholder: string; options?: string[] }> = {
      ...
      walletType: { label: "Wallet Type", type: "select", placeholder: "Select your wallet", 
        options: ["Phantom", "Solflare", "Coinbase Wallet", "Trust Wallet", "MetaMask", "Other"] },
      transactionHash: { label: "Transaction Hash", type: "text", placeholder: "Paste your transaction hash here" },
      amount: { label: "Transaction Amount", type: "text", placeholder: "e.g., 0.1 SOL or $50" },
      squareNumber: { label: "Square Number", type: "text", placeholder: "e.g., Square #25, Row 3 Col 5" },
      gameId: { label: "Game ID", type: "text", placeholder: "Game identifier or week number" },
      username: { label: "Username", type: "text", placeholder: "Your account username" },
      issue: { label: "Describe Your Issue", type: "textarea", placeholder: "Please provide detailed information about the problem you're experiencing..." }
    };
    return configs[fieldName] || { label: fieldName, type: "text", placeholder: "" };
  };
...
export default function HelpPage() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    const fields = ISSUE_TYPES[selectedIssueType as keyof typeof ISSUE_TYPES].fields;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log("Support ticket submitted:", { issueType: selectedIssueType, ...form });
  };
        ...
        <div className="container">
          ...
          {selectedIssueType && !submitted && (
            <div className="support-form">
              <div className="form-header">
                <h2>
                  {ISSUE_TYPES[selectedIssueType as keyof typeof ISSUE_TYPES].icon} 
                  {" "}{ISSUE_TYPES[selectedIssueType as keyof typeof ISSUE_TYPES].name}
                </h2>
              ...
              <form onSubmit={handleSubmit}>
                {ISSUE_TYPES[selectedIssueType as keyof typeof ISSUE_TYPES].fields.map(field => (
                  <DynamicField
                    key={field}
                    field={field}
                    value={form[field] || ""}
          ...
          {submitted && (
            <div className="success-message">
              ...
              <p>
                Thank you! Trainer Reviva has received your <strong>{selectedIssueType}</strong> support request. 
                Coach B will contact you at <strong>{form.email}</strong> with healing solutions.
              </p>
              <p className="ticket-info">
                Your ticket has been logged and prioritized based on the issue type. 
